...
class TrajectoryMonitor(Node):
    def __init__(self):
        ...
        self.create_subscription(
            JointTrajectoryControllerState,                                  # ① 구독 메시지 타입
    ...
    def _state_cb(self, msg: JointTrajectoryControllerState):
        self._error = list(msg.error.positions)     # ② 오차 필드명
    ...
    def _check_convergence(self, idx: int):
        ...
        converged = all(abs(e) < THRESHOLD for e in self._error)  # (제공)
        timed_out = self._elapsed >= TIMEOUT_SEC            # ③ 타임아웃 조건
        ...
        if converged or timed_out:
            ...
            self._wp_idx += 1
            if self._wp_idx >= len(WAYPOINTS):     # ④ 완료 판단
                self.get_logger().info('모든 궤적 완료!')
                raise SystemExit
            self._send_wp(self._wp_idx)           # ⑤ 다음 WP 전송
